...
However, I do not want to relax so much to change maximum saturation feature prior into dark channel prior, but this paper inspired me to re-formulate the third term in similar way. Instead of calculating $L_2$ error in maximum saturation feature domain, I choose to formulate the maximum saturation feature prior in pixel value domain as follows:
\begin{equation}
\begin{split}
\min_{\mathbf{l}} ~&\| \mathbf{b} - \mathbf{l} \otimes \mathbf{k} \|_2^2 + \lambda_1 \|\nabla \mathbf{l}\|_0 + \tau \|\text{W} \odot \mathbf{v} \|_0 \\
&\text{W}(x, y) = \begin{cases}
1, &~~ (x, y) \text{is a maximum pixel} \\
0, &~~ \text{otherwise}
\end{cases}\\
\end{split}
