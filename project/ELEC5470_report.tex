...
I conduct an experiment to test the performance of the original $L_0$ solver and our own implemented $L_1$, $L_2$ solvers. The details are in Section.~\ref{sec:exp}.
...
\end{equation}
where $p$ denotes a small patch with center $(x, y)$. $R$, $G$ and $B$ represent the pixel value of $(x, y)$ in $R, G, B$ channels respectively. If a patch has vivid colors, the value will be small.

\subsubsection{Hybrid Objective Function}
Inspired by the image prior design in \cite{pan2014deblurring}\cite{pan2016blind}, I write the new objective function as follows:
\begin{equation}
...
\end{equation}
Similarly, this objective function can be split into 4 sub-problems:
\begin{equation}
\begin{cases}
...
\label{sec:exp}
To verify our proposed methods, I conduct three experiments:
\begin{itemize}
...
\item \textbf{Noise and parameters}: Study the effects of noise and parameters on the results.
\end{itemize} 
...
\subsection{Noise and Parameters}
