...
As I mentioned in Section.~\ref{sec:introduction}, blind deblurring can be divided two steps: 1) solve Eqn.~\ref{eqn:blind_with_prior_alternate} to get a good kernel. 2) use the solved kernel to get final sharp image by solving Eqn.~\ref{eqn:non_blind_with_prior}. The image prior $\phi(\mathbf{l})$ used in Eqn.~\ref{eqn:non_blind_with_prior} and Eqn.~\ref{eqn:blind_with_prior_alternate} are different, which looks weird. 
